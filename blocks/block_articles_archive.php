...
$limit = Utils::GetComOption("articles","blk_arch_lim",10);
if ($result = $Db->GetList("SELECT YEAR(created) AS year, MONTH(created) AS month, COUNT(id) AS tot FROM #__articles WHERE status='published' GROUP BY year DESC, month DESC LIMIT $limit")) {
