...
class articlesModel extends Views {
	public function _index() {
		...
		if ($year>0) $where[] = "YEAR(a.created)=".intval($year);

		//...by month
		$month = Io::GetVar("GET","month","int",false,0);
		if ($month>0) $where[] = "MONTH(a.created)=".intval($month);

		//...by language
		$lang = Io::GetVar("GET","language","[^a-zA-Z0-9\-]");
		if (!empty($lang)) $where[] = "a.language='".$Db->_e($lang)."'";
		
		//When the plugin is loaded in home, show "inhome" tagged articles only
		if (_ISHOME==1) $where[] = "a.inhome='1'";
		
		$where[] = "a.status='published'";
		$where[] = "NOW() BETWEEN a.start AND a.end";
		if ($flang==1 && empty($lang)) $where[] = "a.language='".$Db->_e($config_sys['language'])."'";

		//Tags
		$tag = Io::GetVar("GET","tag","[^a-zA-Z0-9\-]");
		if (!empty($tag)) {
			$tag_q1 = " JOIN #__tags AS t";
			$tag_q2 = " AND t.controller='".$Db->_e(_PLUGIN_CONTROLLER)."' AND a.id=t.item AND t.name='".$Db->_e($tag)."'";
		...
		if ($result = $Db->GetList("SELECT a.*,c.name AS cname, c.title AS ctitle, u.name AS author_name, l.title AS langtitle, s.name AS sname, s.title AS stitle,
									(SELECT ROUND(SUM(rate)/COUNT(id)) AS rating FROM #__ratings WHERE controller='".$Db->_e(_PLUGIN_CONTROLLER)."' AND item=a.id) AS rating
									FROM #__articles AS a{$index} JOIN #__articles_categories AS c JOIN #__articles_sections AS s JOIN #__user AS u
									JOIN #__languages AS l{$tag_q1} ON a.category=c.id AND s.id=c.section AND a.author=u.uid AND a.language=l.file{$tag_q2}
									{$where}
			...
			$this->Show("articles".__FUNCTION__);
	...
	function _view($name) {
		...
		if ($row = $Db->GetRow("SELECT a.*,c.name AS cname, c.title AS ctitle, u.name AS author_name, l.title AS langtitle, s.name AS sname, s.title AS stitle,
								(SELECT ROUND(SUM(rate)/COUNT(id)) AS rating FROM #__ratings WHERE controller='".$Db->_e(_PLUGIN_CONTROLLER)."' AND item=a.id) AS rating
								FROM #__articles AS a FORCE INDEX(created) JOIN #__articles_categories AS c JOIN #__articles_sections AS s JOIN #__user AS u
								JOIN #__languages AS l ON a.category=c.id AND s.id=c.section AND a.author=u.uid AND a.language=l.file
								WHERE a.name='".$Db->_e($name)."' AND a.status='published'")) {
			$id			= Io::Output($row['id'],"int");
			$category	= Io::Output($row['category'],"int");
			$sname		= Io::Output($row['sname']);
			$stitle		= Io::Output($row['stitle']);
			...
			if ($User->CheckRole($roles) || $User->IsAdmin()) {
				//Increment hits
				$Db->Query("UPDATE #__articles SET hits=hits+1 WHERE id=".intval($id));
				...
				if ($Router->GetOption("related",1)==1) {
					...
					if (sizeof($tagarr)>0) {
						if ($result = $Db->GetList("SELECT a.title,a.name,t.title AS ttag,t.name AS ntag,c.name AS cname,s.name AS sname,YEAR(a.created) AS year,MONTH(a.created) AS month
													FROM #__articles AS a JOIN #__articles_categories AS c JOIN #__articles_sections AS s JOIN #__tags AS t
													ON c.section=s.id AND a.category=c.id AND a.id=t.item AND a.id!=".intval($id)." AND t.controller='".$Db->_e(_PLUGIN_CONTROLLER)."' AND (".implode(" OR ",$tagarr).")
													WHERE a.status='published'
													GROUP BY a.id
													ORDER BY $orrel
													LIMIT $lirel")) {
					...
					$plugin_related['info']['related'] = _t("RELATED_X",MB::strtolower(_t("ARTICLES")));
				} else {
				...
				$Com->info = array("active"		=> $usecomments,
								   ...
								   "item"		=> $id,
								   "numcom"		=> $comments,
								   "url"		=> "index.php?"._NODE."="._PLUGIN."&amp;sec=$sname&amp;cat=$cname&amp;year=$year&amp;month=$month&amp;title=$name");
				...
				$Pag->tot = $comments;
				$Pag->url = "index.php?"._NODE."="._PLUGIN."&amp;sec=$sname&amp;cat=$cname&amp;year=$year&amp;month=$month&amp;title=$name&amp;compage={PAGE}#comments";
				$plugin_pagination = $Pag->Show();
				...
				$Router->breadcrumbs[] = "<span class='sys_breadcrumb'><a href='index.php?"._NODE."="._PLUGIN."&amp;sec=$sname&amp;cat=$cname&amp;year=$year' title='".CleanTitleAtr($year)."'>$year</a></span>";
				$Router->breadcrumbs[] = "<span class='sys_breadcrumb'><a href='index.php?"._NODE."="._PLUGIN."&amp;sec=$sname&amp;cat=$cname&amp;year=$year&amp;month=$month' title='".CleanTitleAtr(Utils::NumToMonth($month))."'>".Utils::NumToMonth($month)."</a></span>";
	...
	public function _print() {
		...
		} else if (!$row = $Db->GetRow("SELECT a.*,c.name AS cname, c.title AS ctitle, u.name AS author_name
										FROM #__articles AS a JOIN #__articles_categories AS c JOIN #__user AS u ON a.category=c.id AND a.author=u.uid
										WHERE a.name='".$Db->_e($name)."'")) {
			$proceed = false;
			$msg = _t("X_NOT_FOUND_OR_INACTIVE",_t("ARTICLE"));
		}
	...
	public function _rss() {
		...
		$ctitle = "";
		if ($result = $Db->GetList("SELECT a.title,a.name,a.created,u.name AS aname,u.email AS aemail,c.name AS cname,c.title AS ctitle,s.name AS sname,s.title AS stitle,a.text,a.created FROM #__articles AS a JOIN #__articles_categories AS c JOIN #__articles_sections AS s JOIN #__user AS u ON c.id=a.category AND a.author=u.uid AND s.id=c.section WHERE a.status='published'{$where} ORDER BY a.created DESC LIMIT ".intval($limit))) {
		...
		foreach ($result as $row) {
			...
			$Rss->Item(array("title"		=> $title,
							 "link"			=> RewriteUrl($config_sys['site_url']."/index.php?"._NODE."="._PLUGIN."&amp;sec=$sname&amp;cat=$cname&amp;year=$year&amp;month=$month&amp;title=$name"),
							 "permalink"	=> RewriteUrl($config_sys['site_url']."/index.php?"._NODE."="._PLUGIN."&amp;sec=$sname&amp;cat=$cname&amp;year=$year&amp;month=$month&amp;title=$name"),
							 "comments"		=> RewriteUrl($config_sys['site_url']."/index.php?"._NODE."="._PLUGIN."&amp;sec=$sname&amp;cat=$cname&amp;year=$year&amp;month=$month&amp;title=$name")."#comments",
		...
		switch ($status) {
			case "send":
				if (Captcha::Check()===true) {
					...
					$article	= Io::GetVar("POST","article");
					...
					if (empty($article)) $errors[] = _t("INTERROR","PAM_e001");
					$row = $Db->GetRow("SELECT a.title,a.text,c.name AS cname,s.name AS sname,a.created
										FROM #__articles AS a JOIN #__articles_categories AS c JOIN #__articles_sections AS s ON c.id=a.category AND s.id=c.section
										WHERE a.status='published' AND a.name='".$Db->_e($article)."' LIMIT 1");
					if (!$row) $errors[] = _t("INTERROR","PAM_e002");
					...
					if (!sizeof($errors)) {
						...
						$emessage .= "<div style=\"font-family:Verdana, Arial, Helvetica, sans-serif; font-size:16px; font-weight:bold;\"><a href=\"".RewriteUrl($config_sys['site_url']."/index.php?"._NODE."=articles&amp;sec=$sname&amp;cat=$cname&amp;year=$year&amp;month=$month&amp;title=$article")."\">$title</a></div><br>\n";
						$emessage .= "<div style=\"font-family:Verdana, Arial, Helvetica, sans-serif; font-size:10px;\">$text...<br><br>[<a href=\"".RewriteUrl($config_sys['site_url']."/index.php?"._NODE."=articles&amp;sec=$sname&amp;cat=$cname&amp;year=$year&amp;month=$month&amp;title=$article")."\">"._t("READ_MORE")."</a>]</div>\n";
						$emessage .= "</body></html>\n";
	...
	public function _comment() {
		...
		if (!sizeof($errors)) {
			...
			$row = $Db->GetRow("SELECT c.name AS category,s.name AS section,a.name AS article,a.created FROM #__articles AS a JOIN #__articles_categories AS c JOIN #__articles_sections AS s ON a.category=c.id AND s.id=c.section WHERE a.id='".intval($item)."' AND a.status='published' LIMIT 1");
			$category = Io::Output($row['category']);
			$section = Io::Output($row['section']);
			$article = Io::Output($row['article']);
			$created = Io::Output($row['created']);
			...
			if ($modresult==0) $Db->Query("UPDATE #__articles SET comments=comments+1 WHERE id='".intval($item)."'");

			$suffixa = ($modresult) ? "&compage=1&mod=$insid" : "" ;
			$suffixb = ($modresult) ? "#comments" : "#comment".$insid ;
			Utils::Redirect(RewriteUrl($config_sys['site_url']._DS."index.php?"._NODE."="._PLUGIN."&sec=$section&cat=$category&year=$year&month=$month&title=$article{$suffixa}").$suffixb);
		} else {
			//Load plugin language
			Language::LoadPluginFile(_PLUGIN_CONTROLLER);
			//Initialize and show site header
	...
	function _delcomment() {
		...
		if ($total) $result = $Db->Query("UPDATE #__articles SET comments=comments-1 WHERE id=".intval($item)) ? 1 : 0 ;
