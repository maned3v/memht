{foreach item=value from=$plugin_blog_index name=cnt}    
	<div class="tpl_blog_title">
    	{if $value.usecomments==1}
    	<div class="tpl_blog_com"><a href="index.php?{$smarty.const._NODE}={$smarty.const._PLUGIN}&amp;cat={$value.cname}&amp;title={$value.name}#comments" title="{t 1=X_COMMENTS 2=$value.comments}" rel="tooltip">{$value.comments}</a></div>
        {/if}
        <div class="title">
            <a href="index.php?{$smarty.const._NODE}={$smarty.const._PLUGIN}&amp;cat={$value.cname}&amp;title={$value.name}" title="{$value.title|CleanTitleAtr}"><strong>{$value.title}</strong></a>
        </div>
    ...
    *}
    ...
    <div class="tpl_blog_body">
        {$value.text}
        {if $value.more}
        	<div class="tpl_blog_more"><a href="index.php?{$smarty.const._NODE}={$smarty.const._PLUGIN}&amp;cat={$value.cname}&amp;title={$value.name}" title="{$value.title|CleanTitleAtr}" rel="tooltip">{t 1=READ_MORE} &raquo;</a></div>
        {/if}
